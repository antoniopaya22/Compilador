...
public class ComprobacionTiposVisitor extends AbstractVisitor {
	...
	public Object visit(For e, Object param) {
		...
		if (!e.getCondicion().getTipo().esLogico())
			e.getCondicion().setTipo(new TipoError(e.getCondicion().getFila(), e.getCondicion().getColumna(),
					"La condición del for debe ser de tipo Logico"));
		e.getCuerpo().stream().forEach(x -> x.accept(this, param));
		return false;
	}
	...
	public Object visit(Ternario e, Object param) {
		...
		e.getExp3().accept(this, param);
		if (!e.getExp1().getTipo().esLogico())
			e.getExp1().setTipo(new TipoError(e.getExp1().getFila(), e.getExp1().getColumna(),
					"La condición del operador ternario debe ser de tipo Logico"));
		e.setTipo(e.getExp2().getTipo().esMayor(e.getExp3().getTipo()));
		if (e.getTipo() == null)
			e.setTipo(new TipoError(e.getFila(), e.getColumna(),
					"Error al establecer el tipo de la operacion ternaria"));
		return null;
	}
