...
asignacion: expresion '=' expresion		{ $$ = new Asignacion(lexico.getLinea(), lexico.getColumna(), $1, $3); }
		  // AMPLIACION ++,--
 		  | expresion '+' '+'			{ 
											$$ = new Asignacion(lexico.getLinea(), lexico.getColumna(), $1,
												   new Aritmetica(lexico.getLinea(), lexico.getColumna(), $1, "+", 
...
// AMPLIACION FOR	 
for: FOR '(' asignacion ';' expresion ';' asignacion ')' '{' lista_sentencias '}' { $$ = new For(lexico.getLinea(), lexico.getColumna(), $3, $5, $7, $10); }
   ;
...
if_else: IF expresion '{' lista_sentencias '}' ELSE '{' lista_sentencias '}'	 		{ $$ = new If(lexico.getLinea(), lexico.getColumna(), $2, $4, $8); }
	   ...
	   | IF '(' expresion ')' '{' lista_sentencias '}'  %prec MENOSQUEELSE				{ $$ = new If(lexico.getLinea(), lexico.getColumna(), $3, $6, new ArrayList<Sentencia>()); }
	   ;
...
dowhile: DO '{' lista_sentencias '}' WHILE '(' expresion ')' %prec MASQUEWHILE  	{ $$ = new DoWhile(lexico.getLinea(), lexico.getColumna(), $7, $3); }
	   ;
...
expresion: expresion '+' expresion                  { $$ = new Aritmetica(lexico.getLinea(), lexico.getColumna(), $1, "+", $3); }
		 ...
		 | expresion XOR expresion					{ $$ = new Logica(lexico.getLinea(), lexico.getColumna(), $1, "^^", $3); }
		 ...
		 //AMPLIACION OP TERNARIO
		 | ternario									{ $$ = $1; }
		 ;
		 
//AMPLIACION OP TERNARIO
ternario: '(' expresion ')' '?' expresion ':' expresion 	{ $$ = new Ternario(lexico.getLinea(), lexico.getColumna(), $2, $5, $7); }
		;
